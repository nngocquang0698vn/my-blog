...
no_toc: true
---
If you're writing Chef cookbooks in a corporate environment, you may be developing against a Chef Supermarket on your internal network. As it is not a service to be exposed externally, the certificate used for HTTPS will be an intranet-only/self-signed cert, and therefore will require you to manually add it to your trust store.
...
We can combine the steps in [my article on extracting certificates][extract-tls-certificate] with writing to the embedded OpenSSL bundle of trusted certs for the ChefDK. Given you want to trust the certificate for the host `internal-supermarket.example.com`, you can run the following:
