...
I'd recommend starting by parallelising the build process, in the Java world, [using Maven](https://cwiki.apache.org/confluence/display/MAVEN/Parallel+builds+in+Maven+3) or [Gradle]({{< ref 2021-03-11-gradle-speed-parallel >}}) as an initial boost to make the local process a bit quicker.

Next, we actually want to parallelise the tests. In Java, this depends on the test tooling you're using, but [JUnit5 can be straightforward to parallelise]({{< ref 2021-03-11-gradle-speed-parallel >}}), and Cucumber has some [good official documentation](https://cucumber.io/docs/guides/parallel-execution/) for setting up parallel execution.
