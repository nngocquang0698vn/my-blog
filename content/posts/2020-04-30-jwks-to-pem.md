---
title: "Extracting `x5c`s from a JSON Web Key Set (JWKS) to PEM files with Ruby"
...
jwks = JSON.parse(ARGF.read)
jwks['keys'].each do |jwk|
  jwk['x5c'].each_with_index do |cert, i|
    pem = OpenSSL::X509::Certificate.new(Base64.decode64(cert))
    File.open("#{jwk['kid']}_#{i}.pem", 'w') do |f|
      f.write(pem.to_s + "\n")
    end
  end
end
