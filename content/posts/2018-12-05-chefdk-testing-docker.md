...
Assuming you have followed the steps in [_Trusting Self-Signed Certificates from the Chef Development Kit_](/posts/2017/08/17/self-signed-certs-chefdk/), you should be able to mount your `trusted_certs` folder:
...
If we've got an internal Supermarket and are operating behind a proxy, we'd want to follow the steps within [_`SSLError` When Running Berkshelf Behind a Proxy_](/posts/2018/02/16/berkshelf-proxy-workaround/) and add our Supermarket's FQDN to our `no_proxy`. I.e. when we're running our ChefSpec tests:
