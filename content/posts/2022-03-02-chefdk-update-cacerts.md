---
title: "Updating the CA Certificate bundle in Chef Installations"
description: "How to update/add new certificates to the Certificate Authority Certificate Bundle in a Chef Development Kit / Workstation installation."
tags:
- blogumentation
- chef
...
---
Installations of the Chef Development Kit and the Chef Workstation include their own bundle of Certificate Authority Certificates. As these are tied to the installation of Chef, an upgrade _generally_ means that you'd need to update Chef.
...
Due to the [Let's Encrypt root certificate expiry](https://scotthelme.co.uk/lets-encrypt-old-root-expiration/), a number of domains in 2022 will  no longer be trusted by this version of Chef, as the CA Certs don't include the new root certificate.
...
In this specific case of the Let's Encrypt, we'd _also_ need to remove the other CA certs that have expired, and once we do, HTTPS calls succeed!
...
As noted in the cert bundle, we're using the bundle from Mozilla, which can either be extracted yourself, or using the [pre-extracted bundle that curl provides](https://curl.se/docs/caextract.html).
