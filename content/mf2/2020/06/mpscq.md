{
  "date" : "2020-06-15T22:02:00+01:00",
  "deleted" : false,
  "h" : "h-entry",
  "properties" : {
    "syndication" : [ "https://twitter.com/shanselman/status/1272596490725126144" ],
    "name" : [ "Like of @pascaldoesgo's tweet" ],
    "published" : [ "2020-06-15T22:02:00+01:00" ],
    "like-of" : [ "https://twitter.com/pascaldoesgo/status/1272631376832069633" ]
  },
  "kind" : "likes",
  "slug" : "2020/06/mpscq",
  "context" : {
    "type" : [ "h-entry" ],
    "properties" : {
      "uid" : [ "tag:twitter.com:1272631376832069633" ],
      "url" : [ "https://twitter.com/pascaldoesgo/status/1272631376832069633" ],
      "published" : [ "2020-06-15T20:45:39+00:00" ],
      "author" : [ {
        "type" : [ "h-card" ],
        "properties" : {
          "uid" : [ "tag:twitter.com:pascaldoesgo" ],
          "numeric-id" : [ "1003529600901767168" ],
          "name" : [ "Pascal Dennerly" ],
          "nickname" : [ "pascaldoesgo" ],
          "url" : [ "https://twitter.com/pascaldoesgo" ],
          "published" : [ "2018-06-04T06:51:16+00:00" ],
          "location" : [ {
            "type" : [ "h-card", "p-location" ],
            "properties" : {
              "name" : [ "Nottingham, England" ]
            }
          } ],
          "photo" : [ "https://pbs.twimg.com/profile_images/1199757075561222147/eYBkMUrZ.jpg" ]
        }
      } ],
      "content" : [ "I literally wrote a tool to help with this. But not all lower case. That's just weird!" ]
    },
    "children" : [ {
      "type" : [ "u-quotation-of", "h-cite" ],
      "properties" : {
        "uid" : [ "tag:twitter.com:1272596490725126144" ],
        "url" : [ "https://twitter.com/shanselman/status/1272596490725126144" ],
        "published" : [ "2020-06-15T18:27:01+00:00" ],
        "author" : [ {
          "type" : [ "h-card" ],
          "properties" : {
            "uid" : [ "tag:twitter.com:shanselman" ],
            "numeric-id" : [ "5676102" ],
            "name" : [ "Scott Hanselman" ],
            "nickname" : [ "shanselman" ],
            "url" : [ "https://twitter.com/shanselman", "http://hanselman.com" ],
            "published" : [ "2007-05-01T05:55:26+00:00" ],
            "location" : [ {
              "type" : [ "h-card", "p-location" ],
              "properties" : {
                "name" : [ "Portland, Oregon" ]
              }
            } ],
            "photo" : [ "https://pbs.twimg.com/profile_images/1133122333290291200/xV9gO-D6.jpg" ]
          }
        } ],
        "content" : [ {
          "value" : "URL styles\n\nPascalUrlsAreLikeThis\ncamelCaseIsLikeThis\nsnake_case_is_like_this\nkebab-urls-have-a-stick-through-them\nlowercaseurlsarejustweird",
          "html" : "<div style=\"white-space: pre\">URL styles\n\nPascalUrlsAreLikeThis\ncamelCaseIsLikeThis\nsnake_case_is_like_this\nkebab-urls-have-a-stick-through-them\nlowercaseurlsarejustweird</div>"
        } ]
      }
    } ]
  },
  "client_id" : "https://indigenous.realize.be"
}
